...
import { Phone, PhoneOff, ArrowRight, MapPin, Briefcase, Zap, Target, Activity } from 'lucide-react';
...
const WelcomeFlow: React.FC<WelcomeFlowProps> = ({ onComplete }) => {
  ...
  const mockProfile: PublicProfile = {
    ...
    lifeTimeline: [
      { id: 't1', year: '2020', title: '初次探索', description: '开始尝试记录自己的生活点滴' },
      { id: 't2', year: '2022', title: '职业转型', description: '从传统行业跨入互联网运营' },
      { id: 't3', year: '2024', title: '觉醒时刻', description: '意识到反思对于个人成长的重要性' }
    ],
    reflections: []
  };
    ...
    try {
      ...
      const sessionPromise = ai.live.connect({
        ...
        config: {
          ...
          systemInstruction: '你叫 On Her Way 的向导。你正在迎接一位新用户。你的语气应该非常自然、温和、充满鼓励。现在你正在问用户：“想在这个App里玩点什么？” 引导他们自由表达。',
        }
      });
    ...
    <div className="fixed inset-0 z-[1000] bg-slate-50 flex items-center justify-center overflow-hidden font-['Plus_Jakarta_Sans']">
      <AnimatePresence mode="wait">
        {stage === 'welcome' && (
          ...
          >
            <h1 className="text-7xl font-black text-slate-900 mb-6 tracking-tighter">On Her Way</h1>
            <p className="text-slate-400 mb-14 text-xl font-medium tracking-tight opacity-80">重构你的生活拼图</p>
            <button 
        ...
        {stage === 'incoming_call' && (
          ...
          >
            ...
            <div className="text-center space-y-4">
              <h2 className="text-3xl font-black text-slate-900 leading-tight">hi！欢迎来到 on her way!</h2>
              <p className="text-slate-500 font-bold text-lg leading-relaxed opacity-70 px-4">我们先随便聊几句，互相认识一下彼此？</p>
            </div>
            <button 
              onClick={startCall}
          ...
          >
            <div className="flex-1 flex flex-col items-center justify-center max-w-4xl w-full text-center gap-16">
              ...
              <div className="min-h-[80px] max-w-2xl px-8">
                <p className="text-2xl text-slate-400 font-bold italic leading-relaxed opacity-60">
                  {userTranscription || "正在倾听你的声音..."}
                </p>
              </div>
          ...
          >
            <div className="bg-white rounded-[4rem] p-16 shadow-[0_50px_100px_rgba(0,0,0,0.08)] border border-slate-100 flex flex-col md:flex-row gap-24 items-start relative overflow-hidden">
               ...
               <div className="flex flex-col items-center md:items-start text-center md:text-left min-w-[320px] z-10">
                  ...
                  <h3 className="text-5xl font-black text-slate-900 mb-10 tracking-tight">{mockProfile.name}</h3>
                  <div className="space-y-8 w-full">
                    {[
                      { icon: <Briefcase className="w-6 h-6" />, label: "身份背景", val: mockProfile.tags.role_detail, color: "blue" },
                      { icon: <MapPin className="w-6 h-6" />, label: "所在地", val: mockProfile.tags.location, color: "slate" },
                      { icon: <Activity className="w-6 h-6" />, label: "核心体验", val: mockProfile.tags.experience, color: "green" },
                      { icon: <Zap className="w-6 h-6" />, label: "生活困扰", val: mockProfile.tags.hassle, color: "orange" },
                      { icon: <Target className="w-6 h-6" />, label: "成长目标", val: mockProfile.tags.goal, color: "blue" },
                    ].map((item, i) => (
               ...
               <div className="flex-1 w-full z-10 pt-4">
                  <div className="flex items-center gap-4 mb-14">
                    <h4 className="text-xs font-black uppercase tracking-[0.4em] text-slate-400">你的个人经历 / Journey</h4>
                  ...
                  <div className="relative pl-14 border-l-2 border-dashed border-slate-200 space-y-20">
                    {mockProfile.lifeTimeline.map((item) => (
                      <div key={item.id} className="relative group">
                        ...
                        <span className="text-sm font-black text-blue-500 mb-3 block tracking-wider">{item.year}</span>
                        <h5 className="text-2xl font-black text-slate-900 mb-3">{item.title}</h5>
                  ...
                  <div className="mt-28 flex justify-end">
                    ...
                    >
                      开启拼图世界 <ArrowRight className="w-6 h-6 group-hover:translate-x-2 transition-transform" />
                    </button>
                  </div>
               </div>
            </div>
